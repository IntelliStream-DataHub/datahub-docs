...
- **Trace a path**, the rest dims and one route stands out, which is how you follow a
...
- **Vague relationship names.** A model full of `relatedTo` edges is **a picture, not a
...
- **Orphans.** A resource with no relationships is invisible to every traversal. In
  DataHub, a [resource](/using/resources) left with no relationships at all is removed,
  which is a deliberate nudge: things in the model should be connected to something.
...
- [Cypher, the query language DataHub uses for traversal](https://en.wikipedia.org/wiki/Cypher_(query_language))
