...
integration that does is a small bridge you run, subscribing to the topics you choose and
pushing through the same [API and SDKs](/using/timeseries#getting-data-in) every other source
uses. The SDKs carry the batching, retries and durable buffering that bridge needs, so it is
