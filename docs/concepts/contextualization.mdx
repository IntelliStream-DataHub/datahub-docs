...
```mermaid
graph TD
    H["Historian<br/>tag COM-99-PT-1034"]
...
<Steps>
  <Step title="Match on the identifiers you already have">
    A plant tag such as `COM-99-PT-1034` already encodes facility, system and instrument.
    ...
    Mirroring it is literal: DataHub stores the tag
    [exactly as you send it](/concepts/naming-and-standards#what-an-external-id-may-contain),
    uppercase, hyphens and all. Every system then holds the same string, so matching is a
    straight comparison rather than a normalisation each integration has to reimplement.
  </Step>
...
On the roadmap, this is also where assisted classification lands: suggesting which class and
connections new or unfamiliar data should get, so the human confirms instead of constructs.
[AI agents and integration work →](/value/ai-agents#agents-that-build-and-run-your-integrations)

## Where agents help

This is the single best match between the work and the tool. Deciding that `21-PT-3105`
belongs to the separator on train two is a judgement made from naming patterns, from the
source system's own hierarchy, from what the signal actually does, and from what is already
in the graph. That is pattern-matching over messy evidence, at a scale nobody wants to do by
hand, which is exactly what a model is good at.

Two conditions keep it safe. Have the agent **propose** links rather than commit them, at
least until you have watched a few hundred of its suggestions, and have it record its
reasoning so a rejected suggestion teaches you something about the naming rather than
vanishing. [Building AI agents →](/using/ai-agents)
