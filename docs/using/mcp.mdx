...
| `timeseries_fetch_datapoints` | `externalId` or `id`; `start`, `end` (ISO-8601 or epoch milliseconds, end exclusive); `limit` (default 1000, hard max 100000); `aggregates` and `granularity` | Raw samples, or, with `aggregates` of `avg`, `sum`, `min`, `max` and a bucket such as `1h`, one value per bucket |

Timestamps come back as ISO-8601 strings, the same as the REST API, so a value reads the same
whichever way an assistant reached it. On the way in, either form is accepted: ISO-8601, or an
...
| `timeseries_send_datapoint` | `externalId` or `id`; `timestamp` (ISO-8601, or an epoch in seconds or milliseconds); `value` as a string | The insert result. One datapoint per call; the bulk path is the API |
...
- **It bounds itself.** A limited number of model-to-tool round trips per question, oversized
  tool results truncated with a note telling the model to narrow its query, and a capped
  conversation history. The round trips, the answer length and how hard it thinks are yours
  to set, because the model is billed to you.
...
- **You bring the model.** Your tenant supplies the provider, the model and the API key, and
  the usage is billed to you. There is no platform-wide model behind it.
- **Three things must agree** before it appears at all: the feature is enabled for your
  tenant, your tenant has configured a model, and the signed-in user holds the
  `DATAHUB_CHAT` role.
