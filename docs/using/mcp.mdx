...
| `timeseries_fetch_datapoints` | `externalId` or `id`; `start`, `end` (ISO-8601 or an epoch, end exclusive); `limit` (default 1000, hard max 100000); `aggregates` and `granularity` | Raw samples, or, with `aggregates` of `avg`, `sum`, `min`, `max` and a bucket such as `1h`, one value per bucket |
