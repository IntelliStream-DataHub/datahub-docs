...
The service also offers the analysis to
[AI agents](/value/ai-agents#how-an-agent-actually-connects) as a tool, using the token
validation it already performs, so exposing it needs no extra configuration. The console's
assistant reaches it through the same address setting the Analyze tab uses, and copes with
its absence: if the service is not deployed, the assistant keeps working and that one tool
