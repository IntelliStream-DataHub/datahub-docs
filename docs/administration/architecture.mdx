...
It also keeps the **knowledge graph** in step with the model. A model change is queued in
the relational database in the same transaction as the change itself, applied to the graph
moments after it commits, and retried until it lands, so a restart or a graph database
outage delays the mirror rather than losing a change. Any API instance can do this work,
so nothing needs to run single-instance for the graph. The mirror is eventually
consistent: normally within milliseconds, so a read straight after a write can briefly
predate it.
