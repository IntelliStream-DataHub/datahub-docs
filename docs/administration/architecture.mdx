...
on its own `/mcp`, using the token validation it already performs, so exposing it needs no
extra configuration. The console's assistant reaches it through the same address setting the
